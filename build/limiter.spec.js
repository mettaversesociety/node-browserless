...
describe(`Limiter`, () => {
    ...
    it(`won't add items to the queue when reached`, () => new Promise((r) => {
        const config = new Config();
        const monitoring = new Monitoring(config);
        const metrics = new Metrics();
        config.setConcurrent(1);
    ...
    it(`won't add items when health is bad`, () => new Promise((r) => {
        const config = new Config();
    ...
    it(`will add items when health is good`, () => new Promise((r) => {
        const config = new Config();
        config.enableHealthChecks(true);
