...
describe('WebSocket API', function () {
    ...
    it('runs chromium CDP requests', async () => {
        ...
        const metrics = new Metrics();
        await start({ config, metrics });
        const browser = await chromium.connectOverCDP(`ws://localhost:3000?token=browserless`);
        await browser.close();
    });
    it('runs chromium websocket requests', async () => {
        const config = new Config();
    ...
    it('runs with ignored arguments', async () => {
        ...
        const success = await puppeteer
            .connect({
            browserWSEndpoint: `ws://localhost:3000?token=browserless&launch=${JSON.stringify(args)}`,
        })
            .then(async (b) => {
            const page = await b.newPage();
            await page.close();
    ...
    it('runs playwright', async () => {
        ...
        const metrics = new Metrics();
        await start({ config, metrics });
        const browser = await chromium.connect(`ws://localhost:3000/playwright/chromium?token=browserless`);
        await browser.close();
        await sleep(100);
        const results = metrics.get();
